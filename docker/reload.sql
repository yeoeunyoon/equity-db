-- reload.sql
-- Re-runnable full reload of the Equity Market Intelligence database from the
-- current data/*.tsv files. Used by the `refresh` service after each
-- generate_data.py run (unlike the init-only load.docker.sql, this clears the
-- tables first so it can run repeatedly).
--
-- Must be run as a user with the FILE privilege (root) because it uses
-- server-side LOAD DATA INFILE from /var/lib/mysql-files/data (secure_file_priv).

USE equity_db;

SET FOREIGN_KEY_CHECKS = 0;

-- Clear existing rows (DELETE, not TRUNCATE, to stay valid on FK-referenced
-- parent tables; ids are supplied explicitly by the TSVs so AUTO_INCREMENT
-- does not need resetting).
DELETE FROM Corporate_Action;
DELETE FROM Financial_Snapshot;
DELETE FROM Price;
DELETE FROM Holding;
DELETE FROM Option_Contract;
DELETE FROM ETF;
DELETE FROM Stock;
